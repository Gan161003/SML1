import sys
import re
from pathlib import Path

CURRENT_DIR = Path(__file__).resolve().parent

if str(CURRENT_DIR) not in sys.path:
    sys.path.append(str(CURRENT_DIR))

import pandas as pd
from textblob import TextBlob

from base_cleaner import (
    safe_fillna,
    remove_duplicates,
    clean_text,
    get_word_count
)

# =====================================================
# CONFIG
# =====================================================

RAW_FOLDER = Path(
    "backend/storage/raw/youtube/comments"
)

OUTPUT_FOLDER = Path(
    "backend/storage/cleaned/youtube/comments"
)

# =====================================================
# SENTIMENT
# =====================================================

def get_sentiment_score(text):

    try:

        return round(
            TextBlob(
                str(text)
            ).sentiment.polarity,
            4
        )

    except:

        return 0.0


def get_sentiment_label(score):

    if score > 0.1:
        return "Positive"

    elif score < -0.1:
        return "Negative"

    return "Neutral"


def get_subjectivity_score(text):

    try:

        return round(
            TextBlob(
                str(text)
            ).sentiment.subjectivity,
            4
        )

    except:

        return 0.0


# =====================================================
# HASHTAGS
# =====================================================

def extract_hashtags(text):

    try:

        tags = re.findall(
            r"#(\w+)",
            str(text)
        )

        return ",".join(
            list(set(tags))
        )

    except:

        return ""


def hashtag_count(text):

    try:

        return len(
            re.findall(
                r"#(\w+)",
                str(text)
            )
        )

    except:

        return 0


# =====================================================
# BRANDS
# =====================================================

BRANDS = [
    "coca cola",
    "coca-cola",
    "coke",
    "pepsi",
    "sprite",
    "fanta",
    "mirinda",
    "7up",
    "mountain dew"
]


def extract_brand_mentions(text):

    found = []

    text = str(text).lower()

    for brand in BRANDS:

        if brand in text:

            found.append(
                brand
            )

    return ",".join(found)


def count_brands(text):

    count = 0

    text = str(text).lower()

    for brand in BRANDS:

        if brand in text:

            count += 1

    return count


# =====================================================
# INTENT DETECTION
# =====================================================

PURCHASE_WORDS = [
    "buy",
    "order",
    "price",
    "cost",
    "available",
    "where",
    "contact"
]

COMPLAINT_WORDS = [
    "bad",
    "worst",
    "fake",
    "not",
    "poor",
    "hate",
    "disappointed"
]


def is_question(text):

    text = str(text)

    return (
        "?" in text
    )


def is_purchase_intent(text):

    text = str(text).lower()

    for word in PURCHASE_WORDS:

        if word in text:
            return True

    return False


def is_complaint(text):

    text = str(text).lower()

    for word in COMPLAINT_WORDS:

        if word in text:
            return True

    return False


# =====================================================
# QUALITY SCORE
# =====================================================

def quality_score(row):

    score = 0

    if row["comment_word_count"] > 5:
        score += 30

    if row["sentiment_label"]:
        score += 20

    if row["brand_count"] > 0:
        score += 20

    if row["comment_like_count_numeric"] > 0:
        score += 30

    return min(score, 100)


# =====================================================
# URGENCY SCORE
# =====================================================

def urgency_score(row):

    score = 0

    if row["sentiment_label"] == "Negative":
        score += 40

    if row["is_complaint"]:
        score += 40

    if row["is_question"]:
        score += 20

    return min(score, 100)


# =====================================================
# FILE CLEANER
# =====================================================

def clean_youtube_comments_file(
    input_file,
    output_file
):

    print(
        f"Processing: {input_file.name}"
    )

    df = pd.read_csv(
        input_file
    )

    df = safe_fillna(df)

    df = remove_duplicates(df)

    # =====================================
    # CLEAN COMMENT
    # =====================================

    df["clean_comment"] = (
        df["comment_text"]
        .astype(str)
        .apply(clean_text)
    )

    df["comment_word_count"] = (
        df["clean_comment"]
        .apply(get_word_count)
    )

    df["comment_char_count"] = (
        df["clean_comment"]
        .str.len()
    )

    # =====================================
    # SENTIMENT
    # =====================================

    df["sentiment_score"] = (
        df["clean_comment"]
        .apply(
            get_sentiment_score
        )
    )

    df["sentiment_label"] = (
        df["sentiment_score"]
        .apply(
            get_sentiment_label
        )
    )

    df["subjectivity_score"] = (
        df["clean_comment"]
        .apply(
            get_subjectivity_score
        )
    )

    # =====================================
    # COMMENT AGE
    # =====================================

    comment_date = pd.to_datetime(
        df["comment_published_at"],
        utc=True,
        errors="coerce"
    )

    current_time = pd.Timestamp.now(
        tz="UTC"
    )

    age = (
        current_time
        - comment_date
    )

    df["comment_age_days"] = (
        age.dt.days
    )

    df["comment_age_hours"] = (
        age.dt.total_seconds()
        / 3600
    ).round(2)

    # =====================================
    # HASHTAGS
    # =====================================

    df["derived_hashtags"] = (
        df["video_title"]
        .astype(str)
        .apply(
            extract_hashtags
        )
    )

    df["derived_hashtag_count"] = (
        df["video_title"]
        .astype(str)
        .apply(
            hashtag_count
        )
    )

    # =====================================
    # BRANDS
    # =====================================

    combined_text = (
        df["video_title"].astype(str)
        +
        " "
        +
        df["clean_comment"].astype(str)
    )

    df["brand_mentions"] = (
        combined_text
        .apply(
            extract_brand_mentions
        )
    )

    df["brand_count"] = (
        combined_text
        .apply(
            count_brands
        )
    )

    # =====================================
    # INTENT
    # =====================================

    df["is_question"] = (
        df["comment_text"]
        .apply(
            is_question
        )
    )

    df["is_purchase_intent"] = (
        df["comment_text"]
        .apply(
            is_purchase_intent
        )
    )

    df["is_complaint"] = (
        df["comment_text"]
        .apply(
            is_complaint
        )
    )

    # =====================================
    # LIKES
    # =====================================

    df["comment_like_count_numeric"] = pd.to_numeric(
        df["comment_like_count"],
        errors="coerce"
    ).fillna(0)

    df["engagement_score"] = (
        df["comment_like_count_numeric"]
    )

    # =====================================
    # QUALITY
    # =====================================

    df["quality_score"] = (
        df.apply(
            quality_score,
            axis=1
        )
    )

    # =====================================
    # URGENCY
    # =====================================

    df["urgency_score"] = (
        df.apply(
            urgency_score,
            axis=1
        )
    )

    # =====================================
    # SAVE
    # =====================================

    output_file.parent.mkdir(
        parents=True,
        exist_ok=True
    )

    df.to_csv(
        output_file,
        index=False
    )

    print(
        f"Saved: {output_file.name}"
    )


# =====================================================
# RUN
# =====================================================

def process_youtube_comments():

    OUTPUT_FOLDER.mkdir(
        parents=True,
        exist_ok=True
    )

    files = list(
        RAW_FOLDER.glob(
            "*.csv"
        )
    )

    print(
        f"Files Found: {len(files)}"
    )

    for file in files:

        try:

            output_file = (
                OUTPUT_FOLDER
                / file.name
            )

            clean_youtube_comments_file(
                file,
                output_file
            )

        except Exception as e:

            print(
                f"Failed: {file.name}"
            )

            print(str(e))


if __name__ == "__main__":

    process_youtube_comments()